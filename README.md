...
- **신규 유틸 `src/common/utils/global-patient.ts`**: 지역(시도/시군구) 영문·중문·일문 매핑 + 13개 진료과 다국어 명칭 + 30여 개 시술 다국어 매핑 → 병원 정보만으로 영어 4~5개 + 중국어/일본어 각 1개의 외국인 환자 질문 자동 생성
- **온보딩 자동 질문 ⑩ 카테고리**: `createAutoPrompts`에 외국인 환자 관점 추가. STANDARD 2개 / PRO 4개 / ENTERPRISE 6개 (FREE·STARTER는 슬롯이 작아 국내 질문 우선). 외국어 질문이 국내 질문에 밀려 잘리지 않도록 **슬롯 20% 예약** 방식
- **Daily Prompt Matrix `global` 톤 축**: 일일 10개 자동 생성 후보에 외국어 질문 포함 (톤 다양성 버킷 덕분에 하루 최소 1개 선택 가능). 영어 텍스트용 **단어 단위 Jaccard 유사도** 추가 (문자 단위로는 알파벳 26자 공유 탓에 전부 중복 판정되는 오탐 수정)
- **언급 감지**: 데모 병원에 다국어 별칭 등록 (`Seoul BD Dental Clinic`, `首尔BD牙科`, `ソウルBD歯科` 등) — 기존 `checkMentionWithVariants`의 lowercase-includes 매칭으로 영어 응답 내 언급 자동 인식 ✅
- **데모 데이터**: 서울비디치과에 외국어 질문 7개(EN 5 + ZH 1 + JA 1) + 31일치 외국어 AI 응답 538건 실DB 반영 (언급률 46%, 성장 트렌드). seed.ts / seed-fast.ts에도 반영 (재시드 시 유지)
- 검증: tsc ✅ / jest 67개 전체 통과 ✅

## 2026.07.04 수천 클라이언트 부하 실측 검증 + 동시성 하드닝
**실데이터 부하 검증** (로컬 PostgreSQL 15에 프로덕션 규모 합성 데이터 생성 후 실측):
- 데이터 규모: 병원 2,000곳 / 프롬프트 16,000개 / **AI응답 288만 행(1.5GB)** / 일일점수 6만 행 / 크롤잡 6만 행
- 대시보드 SoV 쿼리(병원 1곳·30일·1,440행): **10.6ms** — 인덱스 정타 ✅
- Opportunity·Citation·sourceHints·페이지네이션: **0.1~1.4ms** ✅
- 크롤잡 월 한도 카운트: **0.17ms** (양쪽 인덱스 Bitmap AND) ✅
- 동시 50클라이언트 × 500쿼리: 평균 26ms/쿼리, 에러 0 ✅
- 전체 랭킹 집계: 60~130ms — **병원마다 반복 실행되던 것을 전역 캐시(10분)로 전환** (아래)

**동시성(레이스 컨디션) 하드닝**:
- **쿠폰 한도 초과 발급 차단**: `validateCoupon` 사전 체크는 read-then-write라 동시 요청 시 maxUses 초과 발급 가능 → 트랜잭션 내 조건부 `updateMany`(WHERE currentUses < maxUses)로 DB 레벨 원자 차단, count=0이면 롤백. **PostgreSQL 실검증: 동시 20요청 → 정확히 5개만 성공** ✅
- **크론 분산 락**: 수평 확장(인스턴스 2대+) 시 `@Cron` 7개(트라이얼 만료·구독 만료·이메일 4종·경쟁사 감지)가 전 인스턴스에서 동시 발화 → 이메일 중복 발송·이중 다운그레이드 위험. Redis `SET NX` 락(`acquireLock`)으로 첫 인스턴스만 실행
- **전체 랭킹 전역 캐시**: `getRanking`의 전체 병원 스캔+집계는 병원 무관 공유 데이터 → `ps:global:ranking-base` 키로 10분 캐시. 수천 병원 아침 동시 접속 시 동일 계산 수천 회 → 10분당 1회로 축소
- **결제 멱등성 확인**: `orderId`/`paymentKey` DB unique + DONE 상태 체크로 중복 승인 안전 (기존 구조 검증 완료)
- **테스트 67개** (+10: 쿠폰 소진 레이스 5종, 분산 락 TTL/독립성 3종, 캐시 무효화 키 매칭, getOrSet 단일 실행)
