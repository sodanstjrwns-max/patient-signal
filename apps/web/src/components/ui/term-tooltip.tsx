...
export const TERMS: Record<string, { title: string; desc: string }> = {
  ...
  // ─────────────────────────────────────────────────────────
  // ABHS 점수 체계 (강의록: 노출은 목적이 아니라 예약으로 가는 경로다)
  // ─────────────────────────────────────────────────────────
  abhs: {
    title: 'ABHS 종합 점수',
  ...
  intentFear: {
    title: '두려움 의도',
    desc: '"아프지 않나요", "부작용", "실패하면" 같은 불안 기반 질문. 강의록의 핵심 — 환자의 두려움을 먼저 해소한 병원이 선택됩니다. 경쟁사가 잘 놓치는 무주공산 구간입니다.',
  },
  abhsContribution: {
    title: '응답 기여도',
  ...
  contentGap: {
    title: '콘텐츠 갭',
    desc: '경쟁사는 노출되는데 우리는 안 되는 주제. 강의록의 "무주공산" 개념 — 아무도 제대로 답하지 않은 질문을 우리가 먼저 채우면 가장 싸게 이깁니다.',
  },
  opportunityScore: {
    title: '기회 점수',
  ...
  pbnSuspect: {
    title: '위성 사이트 의심',
    desc: '인용 링크를 몰아주기 위해 급조된 것으로 보이는 도메인(PBN). 잠깐 순위가 오를 수 있으나 AI가 신뢰를 회수하면 그동안 쌓은 게 함께 사라집니다. 강의록의 "하지마 구역"입니다.',
  },
  newChannel: {
    title: '신규 채널',
  ...
  crawlability: {
    title: '크롤 가능성',
    desc: 'AI 로봇이 우리 홈페이지를 읽을 수 있는 상태인지. 강의록의 핵심 순서 — 순위를 올리기 전에 먼저 "읽을 수 있게" 만들어야 합니다. 못 읽으면 순위는 존재하지 않습니다.',
  },
