...
export function AnimatedNumber({
  ...
  return (
    <span className={cn("tabular-nums", className)}>
      <span className="sr-only">{target.toFixed(precision)}</span>
      <span aria-hidden="true">{display.toFixed(precision)}</span>
    </span>
  );
}
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        animation = element.animate(
          ...
          {
            duration: 560,
            delay: Math.min(400, Math.max(0, delay)),
...
/** Stable surface wrapper shared by the product views. */
export function SignalSurface({
  ...
  return <div className={cn("signal-surface", className)}>{children}</div>;
}
