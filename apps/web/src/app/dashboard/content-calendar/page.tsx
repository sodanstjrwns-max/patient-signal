...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  PLANNED: { label: '계획', color: 'bg-[#ededf4] text-[#606070]', icon: Clock },
  ANALYZED: { label: '분석 완료', color: 'bg-[#ededf4] text-[#111118]', icon: Search },
      ...
      <main className="mx-auto max-w-[1440px] space-y-7 px-5 py-7 sm:px-8 xl:px-10">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 flex-1">
            <div className="bg-white rounded-lg border border-[#dedee8] px-3 py-2.5 text-center">
              <p className="text-lg font-bold text-[#353143]">{calendar?.total || 0}</p>
              <p className="text-[10px] text-[#737382]">총 주차</p>
            </div>
            <div className="bg-white rounded-lg border border-[#dedee8] px-3 py-2.5 text-center">
              <p className="text-lg font-bold text-brand-600">{stats.PUBLISHED || 0}</p>
              <p className="text-[10px] text-[#737382]">발행 완료</p>
            </div>
            <div className="bg-white rounded-lg border border-[#dedee8] px-3 py-2.5 text-center">
              <p className="text-lg font-bold text-[#5b4dff]">{stats.ANALYZED || 0}</p>
              <p className="text-[10px] text-[#737382]">분석 완료</p>
            </div>
            <div className="bg-white rounded-lg border border-[#dedee8] px-3 py-2.5 text-center">
              <p className="text-lg font-bold text-[#606070]">{stats.PLANNED || 0}</p>
              <p className="text-[10px] text-[#737382]">계획 중</p>
            </div>
            <div className="bg-white rounded-lg border border-[#dedee8] px-3 py-2.5 text-center">
              <p className="text-lg font-bold text-[#5b4dff]">
                {calendar?.total ? Math.round(((stats.PUBLISHED || 0) / calendar.total) * 100) : 0}%
              </p>
              <p className="text-[10px] text-[#737382]">진행률</p>
            </div>
          </div>
          <Button
        ...
        {calendar?.total > 0 && (
          <Card className="border border-[#dedee8] shadow-none">
            <CardContent className="p-4">
              <h4 className="text-xs font-semibold text-[#737382] mb-2"><TermTip term="funnelStage">퍼널 분포</TermTip></h4>
                ...
                {Object.entries(funnelConfig).map(([stage, config]) => (
                  <button
                    ...
                    className={`text-[10px] px-2 py-0.5 rounded-full transition ${
                      filterFunnel === stage ? config.color + ' ring-2 ring-offset-1' : 'text-[#858592] hover:text-[#606070]'
                    }`}
                  >
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full transition ${!filterStatus ? 'bg-brand-600 text-white' : 'bg-white text-[#606070] hover:bg-[#f4f4f8] border'}`}
          >
            전체
          </button>
          {Object.entries(statusConfig).map(([key, config]) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-full transition flex items-center gap-1 ${
                filterStatus === key ? config.color + ' ring-2 ring-offset-1' : 'bg-white text-[#606070] hover:bg-[#f4f4f8] border'
              }`}
            >
              {config.label}
            </button>
        ...
        {!isLoading && items.length === 0 && (
          <Card className="border border-[#dedee8] shadow-none">
            <CardContent className="p-12 text-center">
              <CalendarDays className="w-16 h-16 text-[#b9b8c9] mx-auto mb-4" />
              <h3 className="text-lg font-bold text-[#545067] mb-2">콘텐츠 캘린더가 비어있습니다</h3>
              <p className="text-sm text-[#737382] mb-6">
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-[#737382]">캘린더 로딩 중...</p>
          </div>
        ) : (
          <div className="space-y-2">
            {displayItems.map((item) => {
              ...
              return (
                <Card
                  key={item.id}
                  className={`border border-[#dedee8] shadow-none border-l-4 ${priorityBorder} ${isThisWeek ? 'ring-2 ring-brand-200' : ''}`}
                >
                  <CardContent className="p-0">
                    <button
                      onClick={() => setExpandedWeek(isExpanded ? null : item.weekNumber)}
                      className="w-full p-4 text-left flex items-center gap-3 hover:bg-[#f4f4f8]/50 transition"
                    >
                      {/* 주차 번호 */}
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                        isThisWeek ? 'bg-brand-600 text-white' : isPast ? 'bg-[#ededf4] text-[#858592]' : 'bg-[#ededf4] text-[#545067]'
                      }`}>
                        {item.weekNumber}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          {item.contentType && item.contentType !== 'BLOG' && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#ededf4] text-[#737382]">{item.contentType}</span>
                          )}
                          {isThisWeek && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-brand-100 text-brand-700 font-bold">이번 주</span>
                          )}
                        </div>
                        <p className="text-sm font-medium text-[#353143] truncate">{item.topic}</p>
                      ...
                      {item.seoDirectives && (
                        <div className="flex-shrink-0">
                          <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#ededf4] text-[#5b4dff] font-medium">
                            SEO 지시어 ✓
                          </span>
                        </div>
                      )}
                    ...
                    {/* 확장 영역 */}
                    {isExpanded && (
                      <div className="px-4 pb-4 border-t bg-[#f4f4f8]/50 space-y-3">
                        {/* 액션 버튼 */}
                        ...
                        {item.seoDirectives && (
                          <div className="bg-white rounded-lg p-3 border">
                            <h5 className="text-xs font-semibold text-[#111118] mb-2 flex items-center gap-1">
                        ...
                        {/* 메모 */}
                        {item.notes && (
                          <p className="text-xs text-[#737382] italic">📝 {item.notes}</p>
                        )}
                      </div>
                    )}
                  </CardContent>
