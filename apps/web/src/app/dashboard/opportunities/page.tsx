...
const urgencyConfig = {
  ...
  low: { label: '검토', color: 'blue', icon: Lightbulb, bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-700' },
};
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500">긴급 기회</p>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500">중요 기회</p>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500">Content Gap</p>
                <p className="text-2xl font-bold text-gray-900">{contentGaps.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500">전체 기회</p>
                <p className="text-2xl font-bold text-gray-900">{opportunities.length + contentGaps.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          {sortedOpportunities.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-500 font-medium">현재 발견된 노출 기회가 없습니다</p>
            ...
            sortedOpportunities.map((opp, i) => {
              ...
              return (
                <Card key={i} className={`${config.border} border hover:shadow-md transition-all`}>
                  <CardContent className="p-4">
                    <div 
                      ...
                      <div className="mt-4 pt-4 border-t space-y-3">
                        {opp.suggestedAction && (
                          <div className="bg-blue-50 rounded-lg p-3">
                            ...
                            <p className="text-xs text-blue-600 leading-relaxed">{opp.suggestedAction}</p>
                          </div>
                        )}
                        <div className="grid grid-cols-2 gap-3">
                          <div className="bg-gray-50 rounded-lg p-3">
                            <p className="text-[10px] text-gray-400 mb-1">질문 의도</p>
                            <p className="text-xs font-medium text-gray-700">{opp.intent || '정보 탐색'}</p>
                          </div>
                          <div className="bg-gray-50 rounded-lg p-3">
                            <p className="text-[10px] text-gray-400 mb-1">감지일</p>
          ...
          {contentGaps.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-500 font-medium">Content Gap이 아직 없습니다</p>
                <p className="text-gray-400 text-sm mt-1">
            ...
            contentGaps.map((gap) => {
              ...
              return (
                <Card key={gap.id} className="hover:shadow-md transition-all">
                  <CardContent className="p-4">
                    <div 
                      className="flex items-start justify-between cursor-pointer"
                    ...
                    >
                      <div className="flex items-start gap-3 flex-1">
                        <div className={`p-2 rounded-lg flex-shrink-0 ${
                          gap.priorityScore >= 7 ? 'bg-red-50' : gap.priorityScore >= 4 ? 'bg-amber-50' : 'bg-blue-50'
                        }`}>
                          <Target className={`h-4 w-4 ${
                            gap.priorityScore >= 7 ? 'text-red-600' : gap.priorityScore >= 4 ? 'text-amber-600' : 'text-blue-600'
                          }`} />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className={`px-2 py-0.5 text-[10px] rounded-full border ${
                              ...
                              : gap.gapType === 'KEYWORD' ? 'bg-blue-50 text-blue-600 border-blue-200'
                            ...
                            <div className="flex items-center gap-1">
                              {Array.from({ length: 10 }, (_, i) => (
                                <div key={i} className={`w-1.5 h-1.5 rounded-full ${
                                  i < gap.priorityScore ? 'bg-blue-500' : 'bg-gray-200'
                                }`} />
                              ))}
                            </div>
                          </div>
                    ...
                    {isExpanded && (
                      <div className="mt-4 pt-4 border-t space-y-3">
                        ...
                        {gap.aiGeneratedGuide && (
                          <div className="bg-blue-50 rounded-lg p-3">
                            <p className="text-xs font-semibold text-blue-700 mb-1">🤖 AI 가이드</p>
                            <p className="text-xs text-blue-600 leading-relaxed whitespace-pre-wrap">{gap.aiGeneratedGuide}</p>
                          </div>
                        )}
                      </div>
                    )}
