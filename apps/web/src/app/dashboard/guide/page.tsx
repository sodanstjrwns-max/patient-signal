    ...
    <div className="min-h-screen bg-[#08090a]">
      ...
      <main className="mx-auto max-w-[1440px] space-y-7 px-5 py-7 sm:px-8 xl:px-10">
          ...
          <Card className="border-[#30343a] bg-[#08090a]">
            <CardContent className="pt-6">
              <div className="flex items-start gap-4">
                <div className="p-3 rounded-md bg-[#181b1e] text-[#c0c4c7] shrink-0">
                  <BookOpen className="w-6 h-6" />
                </div>
                <div>
                  <h2 className="font-display text-lg font-semibold text-[#f5f5ef] mb-2">
                    Patient Signal은 &quot;AI 검색 시대의 건강검진&quot;입니다
                  </h2>
                  <p className="text-sm text-[#c0c4c7] leading-relaxed">
                    ...
                    <strong className="text-[#f5f5ef]"> 우리 병원이 등판하는지 · 몇 번째로 불리는지 · 어떤 톤으로 소개되는지</strong>를
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="block text-[11px] font-semibold text-[#959c9f] mt-0.5">
            ...
            <CardContent className="space-y-3">
              {[
                ...
                <Link
                  key={item.step}
                  href={item.href}
                  className="flex items-start gap-3.5 rounded-lg border border-amber-100 bg-[#111315] p-4 hover:border-amber-300 hover:shadow-sm transition-all group"
                >
                  ...
                  <div className="min-w-0">
                    <h3 className="text-sm font-semibold text-[#f5f5ef]">{item.title}</h3>
                    <p className="text-xs text-[#959c9f] leading-relaxed mt-1">{item.desc}</p>
        ...
        <section id="core-terms-section">
          <h2 className="font-display text-xl font-semibold text-[#f5f5ef] mb-1">1. 숫자 읽는 법 — 핵심 4총사</h2>
          <p className="text-sm text-[#959c9f] mb-4">이 네 가지만 구분하면 대시보드의 80%가 읽힙니다.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {CORE_TERMS.map((t) => (
              <Card key={t.name} className="hover:border-[#30343a] transition-shadow">
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-xs font-bold text-[#959c9f]">한 줄 요약: &quot;{t.oneLiner}&quot;</span>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-2">
                  <p className="text-sm text-[#c0c4c7] leading-relaxed">{t.desc}</p>
                  <div className="flex items-start gap-2 text-xs bg-[#08090a] rounded-lg p-2.5 text-[#959c9f]">
          ...
          {/* 30초 요약 박스 */}
          <div className="mt-4 rounded-md border-2 border-dashed border-[#30343a] bg-[#08090a]/50 p-4">
            <p className="text-sm font-bold text-[#f5f5ef]">
              💡 30초 요약: <span className="font-medium text-[#f5f5ef]">
        ...
        <section id="advanced-terms-section">
          <h2 className="font-display text-xl font-semibold text-[#f5f5ef] mb-1">2. 리포트에 나오는 심화 용어</h2>
          <p className="text-sm text-[#959c9f] mb-4">ABHS 리포트·인사이트 페이지에서 만나는 개념들. 각 지표 옆 ⓘ 아이콘에서도 같은 설명을 볼 수 있습니다.</p>
          <Card>
            <CardContent className="pt-6 divide-y divide-[#30343a]">
              {ADVANCED_TERMS.map((t) => (
                <div key={t.name} className="py-3 first:pt-0 last:pb-0">
                  <h3 className="text-sm font-bold text-[#f5f5ef] mb-1">{t.name}</h3>
                  <p className="text-sm text-[#c0c4c7] leading-relaxed">{t.desc}</p>
                </div>
              ))}
            </CardContent>
        ...
        <section id="menu-guide-section">
          <h2 className="font-display text-xl font-semibold text-[#f5f5ef] mb-1">3. 메뉴별 안내 — 어디서 뭘 보나</h2>
          <p className="text-sm text-[#959c9f] mb-4">각 카드를 누르면 해당 메뉴로 이동합니다.</p>
          <div className="space-y-6">
            {MENU_GUIDE.map((group) => (
              <div key={group.group}>
                <h3 className="text-xs font-semibold text-[#959c9f] uppercase tracking-wider mb-2">{group.group}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {group.items.map((item) => (
                    <Link key={item.name} href={item.href} className="block group">
                      <Card className="h-full hover:border-[#30343a] transition-colors">
                        <CardContent className="pt-4 pb-4">
                          <div className="flex items-center gap-2 mb-2">
                            <item.icon className="w-4 h-4 text-[#c0c4c7]" />
                            <span className="font-bold text-sm text-[#f5f5ef] group-hover:text-[#c0c4c7] transition-colors">{item.name}</span>
                          </div>
                          <p className="text-xs text-[#c0c4c7] leading-relaxed mb-2">{item.what}</p>
                          <p className="text-[11px] font-semibold text-[#ff9565]">🕐 {item.when}</p>
                        </CardContent>
                      </Card>
                    </Link>
                  ))}
        ...
        <section id="calendar-guide-section">
          <h2 className="font-display text-xl font-semibold text-[#f5f5ef] mb-1">4. 56주 캘린더 — 이렇게 쓰세요</h2>
          <p className="text-sm text-[#959c9f] mb-4">1년치 콘텐츠 계획을 대신 짜주는 기능입니다.</p>
          <Card>
              ...
              <ol className="space-y-3">
                {[
                  ...
                  <li key={s.step} className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#ff6a24] text-[#08090a] text-xs font-semibold flex items-center justify-center shrink-0 mt-0.5">{s.step}</span>
                    <div>
                      <p className="text-sm font-bold text-[#f5f5ef]">{s.title}</p>
                      <p className="text-sm text-[#c0c4c7] leading-relaxed">{s.desc}</p>
                    </div>
                  </li>
                ))}
              </ol>
        ...
        <section id="routine-section">
          <h2 className="font-display text-xl font-semibold text-[#f5f5ef] mb-1">5. 추천 주간 루틴 — 주 37분이면 충분합니다</h2>
          <p className="text-sm text-[#959c9f] mb-4">매일 다 볼 필요 없습니다. 이 루틴대로만 보세요.</p>
          <Card>
            <CardContent className="pt-6">
              <div className="space-y-3">
                {WEEKLY_ROUTINE.map((r) => (
                  <div key={r.day} className="flex items-start gap-3">
                    ...
                    <div>
                      <span className="text-sm font-bold text-[#f5f5ef]">{r.day}</span>
                      <p className="text-sm text-[#c0c4c7] leading-relaxed">{r.task}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section id="closing-section" className="pb-8">
          <div className="rounded-lg border border-[#30343a] bg-[#281a13] p-6 text-center">
            <p className="text-sm font-medium text-[#c0c4c7] leading-relaxed">
              지표가 이해되지 않을 때는 각 숫자 옆의 <span className="text-[#ff9565] font-bold">ⓘ 아이콘</span>을 눌러보세요.
