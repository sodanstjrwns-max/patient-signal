...
interface TrendRow { rank: number; name: string; mentions: number; share: number; prevMentions: number; deltaPct: number | null; askedBy: number; platforms: number; topPlatform: string | null; topSido: string | null; variants: string[]; isCustomer: boolean; customerRegion: string | null }
interface TrendData { period: { days: number; since: string; until: string }; filters: { specialty: string | null; sido: string | null }; totalNames: number; totalMentions: number; list: TrendRow[]; risers: TrendRow[]; method: string }
...
export default function TrendingPage() {
  ...
  const delta = (r: TrendRow) => r.deltaPct === null
    ? <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-600"><Flame className="w-3 h-3" />NEW</span>
    : r.deltaPct > 5 ? <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600"><TrendingUp className="w-3 h-3" />+{r.deltaPct}%</span>
    : r.deltaPct < -5 ? <span className="inline-flex items-center gap-1 text-[11px] font-bold text-rose-600"><TrendingDown className="w-3 h-3" />{r.deltaPct}%</span>
        ...
        {data && data.risers.length > 0 && (
          <Card className="border-0 shadow-sm bg-amber-50/60">
            <CardContent className="p-4">
              ...
              <div className="flex flex-wrap gap-2">{data.risers.map((r) => <span key={r.name} className="px-2.5 py-1 rounded-full bg-white border border-amber-200 text-xs font-semibold text-slate-700">{r.name} <span className="text-amber-700">{r.mentions}</span></span>)}</div>
            </CardContent>
          </Card>
        )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.list.map((r) => {
                    ...
                    return (
                      <tr key={r.name} className={`border-t border-slate-100 ${mine ? 'bg-brand-50/60' : ''}`}>
                        ...
                        <td className="px-3 py-2.5 text-right font-bold tabular-nums">{r.mentions.toLocaleString()}</td>
                  ...
                  {data.list.length === 0 && <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-500">이 조건에는 아직 합산할 응답이 없습니다.</td></tr>}
                </tbody>
              </table>
            </div>
            <div className="px-4 py-3 text-[11px] text-slate-500 flex items-start gap-2 border-t border-slate-100"><Info className="w-3.5 h-3.5 mt-0.5 shrink-0" /><span>{data.method} 지역은 "질문한 병원"의 시·도라서 답변 속 병원의 실제 소재지와 다를 수 있습니다. 광고·홍보 목적의 순위가 아니라 AI 답변 관찰 통계입니다.</span></div>
          </Card>
        )}
