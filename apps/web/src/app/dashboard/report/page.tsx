...
const depthColors: Record<string, string> = {
  R3: 'bg-green-500', R2: 'bg-blue-500', R1: 'bg-yellow-500', R0: 'bg-red-400',
};
const sentimentLabel = (v: number) => {
  ...
  if (v >= -0.5) return { text: '중립', color: 'text-gray-500', bg: 'bg-gray-50' };
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="bg-white/10 rounded-xl p-4">
              ...
              <p className="text-xs text-blue-200">언급 점유율</p>
            </div>
            <div className="bg-white/10 rounded-xl p-4">
              ...
              <p className="text-xs text-blue-200">{sentimentLabel(abhs?.avgSentimentV2 ?? 0).text}</p>
            </div>
            <div className="bg-white/10 rounded-xl p-4">
              <p className="text-xs text-blue-200">경쟁 점유율</p>
              <p className="text-3xl font-bold">{competitiveShare?.mySharePercent ?? 0}%</p>
              <p className="text-xs text-blue-200">Weighted Share</p>
            </div>
            <div className="bg-white/10 rounded-xl p-4">
              <p className="text-xs text-blue-200">긴급 알림</p>
              <p className="text-3xl font-bold text-red-300">{criticalActions.length}</p>
              <p className="text-xs text-blue-200">즉시 조치 필요</p>
            </div>
          </div>
        </div>
                  ...
                  <div key={platform} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-400">×{platformWeights[platform.toUpperCase()] || data.weight}</span>
                      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {Object.entries(abhs?.intentScores || {}).map(([intent, score]: [string, any]) => {
                ...
                return (
                  <div key={intent} className={`p-4 rounded-lg border ${weight > 1 ? 'border-blue-200 bg-blue-50' : 'border-gray-100'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-gray-500">{intentNames[intent] || intent}</span>
                      {weight > 1 && <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">×{weight}</span>}
                    </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center gap-4">
                  <div className="w-28 text-sm font-semibold text-blue-600">우리 병원</div>
                  <div className="flex-1 bg-gray-100 rounded-full h-5">
                    <div className="bg-blue-500 rounded-full h-5 flex items-center justify-end pr-2" style={{ width: `${Math.max(5, competitiveShare.mySharePercent)}%` }}>
                ...
                {competitiveShare.competitorShares.map((cs: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-4">
                    <div className="w-28 text-sm text-gray-600 truncate">{cs.name}</div>
                    <div className="flex-1 bg-gray-100 rounded-full h-5">
                      <div className="bg-gray-400 rounded-full h-5 flex items-center justify-end pr-2" style={{ width: `${Math.max(5, cs.sharePercent)}%` }}>
