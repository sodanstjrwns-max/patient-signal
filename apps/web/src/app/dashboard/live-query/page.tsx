...
const platformColors: Record<string, string> = {
  ...
  GEMINI: 'bg-[#e9ebe1] text-[#141512]',
  GROK: 'bg-[#d4d6cb] text-[#141512]',
...
const categoryConfig: Record<string, { name: string; icon: any; color: string; bgColor: string; emoji: string }> = {
  ...
  EMOTION:   { name: '감성/경험', icon: Heart, color: 'text-[#44551d]', bgColor: 'bg-[#f1f1eb] border-[#d4d6cb]', emoji: '💝' },
  ...
  COMPARISON:{ name: '비교', icon: GitCompare, color: 'text-[#44551d]', bgColor: 'bg-[#f1f1eb] border-[#d4d6cb]', emoji: '⚖️' },
  GENERAL:   { name: '기타', icon: HelpCircle, color: 'text-[#687253]', bgColor: 'bg-[#f1f1eb] border-[#d4d6cb]', emoji: '📋' },
};
...
const getSentimentIcon = (label: string) => {
  switch (label) {
    ...
    default: return <Minus className="h-4 w-4 text-[#909781]" />;
  }
};
      ...
      <div className="min-h-screen">
        ...
        <div className="p-4 sm:p-6">
          <Card><CardContent className="p-8 sm:p-12 text-center">
            <Bot className="h-12 w-12 text-[#909781] mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-[#141512] mb-2">병원 등록이 필요합니다</h3>
            <p className="text-[#72756a] mb-4">실시간 질문을 사용하려면 먼저 병원 정보를 등록해주세요.</p>
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-[1440px] space-y-6 px-5 py-7 sm:px-8 xl:px-10">
        ...
        {/* 사용량 배너 */}
        {usage && (
          <Card className={`border ${isLimitReached ? 'border-red-200 bg-red-50/50' : usagePercent >= 80 ? 'border-yellow-200 bg-yellow-50/30' : 'border-[#d4d6cb] bg-white '}`}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <Gauge className="h-4 w-4 text-[#72756a]" />
                  <span className="text-sm font-semibold text-[#525849]">오늘 사용량</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${planType === 'PRO' || planType === 'ENTERPRISE' ? 'bg-brand-100 text-brand-700' : planType === 'STANDARD' || planType === 'STARTER' ? 'bg-brand-100 text-brand-700' : 'bg-[#e9ebe1] text-[#687253]'}`}>
                ...
                {usage.isUnlimited ? (
                  ...
                  <span className={`text-sm font-bold ${isLimitReached ? 'text-red-600' : usagePercent >= 80 ? 'text-yellow-600' : 'text-[#33372c]'}`}>{usage.used} / {usage.limit}회</span>
                )}
              </div>
              {!usage.isUnlimited && (
                <div className="w-full bg-[#d4d6cb] rounded-full h-2 mb-2">
              ...
              <div className="flex items-center justify-between">
                <p className={`text-xs ${isLimitReached ? 'text-red-600' : 'text-[#72756a]'}`}>
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-[#33372c]">카테고리별 성과 분석</span>
        ...
        {/* ==================== 질문 영역 ==================== */}
            {/* 질문 입력 카드 */}
            <Card className="overflow-hidden border-[#d4d6cb] border-t-4 border-t-[#141512] bg-white shadow-none">
              <CardContent className="p-5 sm:p-6">
                <div className="flex items-center gap-3 border-b border-[#d4d6cb] pb-5 mb-5">
                  <div className="w-10 h-10 rounded-none bg-[#141512] flex items-center justify-center shadow-none">
                    <Zap className="h-5 w-5 text-white" />
                  </div>
                  <div>
                    <h2 className="text-xl font-semibold tracking-[-0.04em] text-[#141512]">AI에게 질문하기</h2>
                    <p className="text-xs text-[#72756a]">환자가 실제로 물어볼 만한 질문을 입력해보세요</p>
                  </div>
                </div>

                {/* 플랫폼 선택 */}
                <div className="mb-4">
                  <p className="text-xs font-medium text-[#687253] mb-2">질문할 AI 플랫폼</p>
                  <div className="flex flex-wrap gap-2">
                    {(['CHATGPT', 'CLAUDE', 'PERPLEXITY', 'GEMINI', 'GROK', 'CLOVA_X'] as const).map(platform => (
                      <button key={platform} onClick={() => togglePlatform(platform)} aria-pressed={platforms.includes(platform)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${platforms.includes(platform) ? `${platformColors[platform]} ring-2 ring-offset-1 ring-current shadow-sm` : 'bg-[#e9ebe1] text-[#909781] hover:bg-[#d4d6cb]'}`}>
                ...
                {!results && !loading && !isLimitReached && (
                  <div className="mt-4">
                    <p className="text-xs text-[#72756a] mb-2">예시 질문</p>
                    <div className="flex flex-wrap gap-2">
                      {exampleQuestions.map((q, i) => (
                        <button key={i} onClick={() => { setQuestion(q); handleQuery(q); }}
                          className="text-xs px-3 py-1.5 bg-white border border-[#d4d6cb] rounded-full hover:border-brand-300 hover:bg-brand-50 text-[#687253] hover:text-brand-700 transition-all">
                          {q}
                        </button>
                      ))}
                    </div>
            ...
            {limitReachedError && (
              <Card className="border-red-200 bg-red-50">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-xs text-[#72756a] mb-4">매일 자정(00:00)에 초기화됩니다.</p>
                ...
                <CardContent className="p-8">
                  <div className="flex flex-col items-center justify-center gap-4">
                    ...
                    <div className="text-center">
                      <p className="font-semibold text-[#33372c]">AI에게 질문하는 중...</p>
                      <p className="text-sm text-[#72756a] mt-1">{platforms.map(p => platformNames[p]).join(', ')}에 동시 질문 중</p>
                      <p className="text-xs text-[#909781] mt-2">보통 10~30초 소요</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            {results && !loading && (
              <div className="space-y-4">
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-[#141512]">질문 결과</span>
                        {/* 카테고리 뱃지 */}
                      ...
                      <div className="flex items-center gap-2">
                        {usage && !usage.isUnlimited && (
                          <span className="text-[10px] text-[#909781] bg-[#e9ebe1] px-2 py-0.5 rounded-full">{usage.remaining}회 남음</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div className="bg-[#f1f1eb] rounded-md p-3 text-center">
                        <p className="text-2xl font-bold text-[#33372c]">{results.successCount}<span className="text-sm font-normal text-[#909781]">/{results.totalPlatforms}</span></p>
                        <p className="text-[10px] text-[#72756a] mt-0.5">응답 성공</p>
                      </div>
                      <div className={`rounded-md p-3 text-center ${results.mentionedCount > 0 ? 'bg-brand-50' : 'bg-[#f1f1eb]'}`}>
                        <p className={`text-2xl font-bold ${results.mentionedCount > 0 ? 'text-brand-600' : 'text-[#909781]'}`}>{results.mentionedCount}</p>
                        <p className="text-[10px] text-[#72756a] mt-0.5">우리 병원 언급</p>
                      </div>
                      <div className={`rounded-md p-3 text-center ${results.mentionRate >= 50 ? 'bg-brand-50' : results.mentionRate > 0 ? 'bg-yellow-50' : 'bg-[#f1f1eb]'}`}>
                        <p className={`text-2xl font-bold ${results.mentionRate >= 50 ? 'text-brand-600' : results.mentionRate > 0 ? 'text-yellow-600' : 'text-[#909781]'}`}>{results.mentionRate}%</p>
                        <p className="text-[10px] text-[#72756a] mt-0.5">언급률</p>
                      </div>
                    </div>
                ...
                <div className="space-y-3">
                  <h3 className="text-sm font-bold text-[#525849] flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-brand-500" />플랫폼별 응답
                  </h3>
                  {results.responses?.map((resp: any) => (
                    <Card key={resp.platform} className={`overflow-hidden transition-all ${resp.success && resp.isMentioned ? 'border-l-4 border-l-green-400 border-brand-100' : resp.success ? 'border-[#d4d6cb] hover:border-[#d7dacd]' : 'border-red-200 bg-red-50/30'}`}>
                      <button className="w-full p-4 flex items-center justify-between hover:bg-[#f1f1eb] transition-colors"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {resp.success ? (
                            resp.isMentioned ? <span className="flex items-center gap-1.5 text-sm text-brand-600 font-semibold"><Award className="h-4 w-4" />{resp.mentionPosition ? `${resp.mentionPosition}위 추천` : '언급됨'}</span>
                              : <span className="text-sm text-[#909781]">언급 안됨</span>
                          ) : <span className="text-sm text-red-500 flex items-center gap-1"><XCircle className="h-3.5 w-3.5" />응답 실패</span>}
                        </div>
                        <div className="flex items-center gap-2">
                          {expandedPlatform === resp.platform ? <ChevronUp className="h-4 w-4 text-[#909781]" /> : <ChevronDown className="h-4 w-4 text-[#909781]" />}
                        </div>
                      </button>
                      {expandedPlatform === resp.platform && resp.success && (
                        <div className="px-4 pb-4 border-t border-[#e9ebe1]">
                          <div className="mt-3 text-sm text-[#525849] whitespace-pre-wrap leading-relaxed bg-[#f1f1eb] rounded-md p-4 max-h-[500px] overflow-y-auto">{resp.response}</div>
                          {resp.competitorsMentioned?.length > 0 && (
                            <div className="mt-3 flex flex-wrap gap-1.5 items-center">
                              <span className="text-xs text-[#72756a] mr-1">함께 언급:</span>
              ...
              <Card>
                <CardContent className="p-5">
                  <div className="flex items-center gap-2 mb-4">
                    <History className="h-4 w-4 text-[#72756a]" />
                    <h3 className="text-sm font-bold text-[#525849]">최근 질문 기록</h3>
                  </div>
                  <div className="space-y-2">
                    {history.map((item, idx) => (
                      <button key={idx} className={`w-full text-left p-3 rounded-md transition-all border ${results === item ? 'bg-brand-50 border-brand-200' : 'bg-[#f1f1eb] border-transparent hover:bg-[#e9ebe1]'}`}
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <span className="text-sm text-[#33372c] truncate">{item.question}</span>
                          </div>
                          <span className={`text-xs font-bold min-w-[40px] text-right ${item.mentionRate >= 50 ? 'text-brand-600' : item.mentionRate > 0 ? 'text-yellow-600' : 'text-[#909781]'}`}>{item.mentionRate}%</span>
                        </div>
                      </button>
                    ))}
                  </div>
