...
async function main() {
  ...
  try {
    ...
    // Required outbox schema; fail the build if unavailable.
    await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "book_analytics_events" (
  ...
  "last_error" TEXT
    ...
    await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "book_analytics_events_sent_at_stopped_at_next_attempt_at_idx"
  ON "book_analytics_events"("sent_at", "stopped_at", "next_attempt_at")`)
    console.log('[ensure-sso-columns] book analytics outbox ready')
